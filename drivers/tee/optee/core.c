				   ...
				   struct tee_ioctl_version_data *vers)
{
	...
	if (optee->sec_caps & OPTEE_SMC_SEC_CAP_MEMREF_NULL) /* XXX, mandatory? */
		v.gen_caps |= TEE_GEN_CAP_MEMREF_NULL;
					...
					const struct ffa_dev_ops *ops)
{
	...
	rc = ops->sync_send_receive(ffa_dev, ffa_dev->vm_id, &data);
	if (rc) {
...
static const char *get_conduit_method(struct device *dev)
{
...
{
	const char *method = get_conduit_method(&pdev->dev);
	struct optee *optee;
	int rc;
	...
	optee = optee_probe_legacy(method);
	if (IS_ERR(optee))
