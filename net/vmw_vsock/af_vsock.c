...
static int vsock_sendmsg_shmem_dmabuf(struct vsock_sock *vsk,
				      struct vsock_shmem_user_desc *udesc)
{
	struct dma_buf *dbuf = dma_buf_get(udesc->fd);
	struct vsock_shmem_desc *desc, *old;
	int err;
	...
	if (!dbuf->ops->shmem || !dbuf->ops->unshmem) {
		err = -EOPNOTSUPP;
		goto free_dbuf;
	}
	...
	if (udesc->subop == VSOCK_SHMEM_SUBOP_OFFER) {
		...
		desc = kmalloc(sizeof(*desc) + VSOCK_SHMEM_PAYLOAD_SIZE_MAX,
			       GFP_KERNEL);
		if (!desc) {
			err = -ENOMEM;
			goto free_dbuf;
		...
		/* Share memory and get metadata in `desc` from dmabuf */
		err = dbuf->ops->shmem(dbuf, desc);
		if (err)
			goto free_desc;

		/* Save a copy of descriptor by FD */
		old = xa_store(&vsock_shmem_xa, udesc->fd, desc, GFP_KERNEL);
		if (xa_is_err(old)) {
			err = xa_err(old);
			goto free_desc;
		}
		...
		desc = xa_erase(&vsock_shmem_xa, udesc->fd);
		if (!desc) {
			err = -EINVAL;
			goto free_dbuf;
		}
	...
	if (udesc->subop == VSOCK_SHMEM_SUBOP_RECLAIM) {
		dbuf->ops->unshmem(dbuf);
		kfree(desc);
	}
...
out:
	if (udesc->subop == VSOCK_SHMEM_SUBOP_OFFER) {
		xa_erase(&vsock_shmem_xa, udesc->fd);
free_desc:
		dbuf->ops->unshmem(dbuf);
		kfree(desc);
	}
